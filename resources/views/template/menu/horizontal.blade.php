<div x-data="{ open: false }" class="relative w-full">
    ...
    @if (!isset($type))
        <button @click.prevent="open = ! open"
            class="absolute top-0 right-0 flex items-center gap-3 px-4 py-2 text-gray-600 md:hidden hover:text-teal-500">
    ...
    @foreach ($items as $item)
        @if (count($item->children) > 0)
            <li x-data="{ open: false }" class="relative">
                <button @click.prevent="open = ! open"
                    class="flex items-center gap-3 px-4 py-2 text-gray-600 hover:text-teal-500">{{ $item->title }}
                ...
                <ul x-cloak x-show="open" click.outside="open = false"
                    ...
                    class="relative z-50 flex flex-col mt-2 mb-3 bg-gray-300 rounded-md shadow-sm md:absolute w-44">
            ...
            @if (isset($type) && $type == 'children')
                <li>
                    <a href="{{ isset($item->route) ? route($item->route) : url($item->url) }}"
                        class="block px-4 py-2 text-gray-600 hover:text-teal-500">{{ $item->title }}</a>
                </li>
            @else
                <li>
                    <a href="{{ isset($item->route) ? route($item->route) : url($item->url) }}"
